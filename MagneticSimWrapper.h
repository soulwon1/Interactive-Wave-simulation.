...
public:
	...
	{
		sim->addMagnet(Magnet(x, y, angle, strength, width, height)); //Create a Magnet object and add it to the simulaton 
	...
	{
		double bx, by; 
		sim->computeField(px, py, bx, by, -1);
		Bx = bx;
		By = by; 
	}
...
public: 
	//here we're going to add some interactive functions, this is to turn the simulation more interactive.
	// Do note you dont need this for barebones simulation.

	void SetMagnetPosition(int i, double x, double y) 

		//This was edited to allow us to set the position of a magnet, this is useful for dragging magnets around with the mouse.
		// Note that we also set the velocity to 0 when we do this to prevent the magnet from jumping after dragging.
	{
	...
	{
		if (i < 0 || i >= (int)sim->magnets.size()) return; 
		sim->magnets[i].angle += 3.141596; 
	}

	int AddMagnetAt(double x, double y) //adds a magnet 

	{ //note I had to change this from void to int due to issues with the game and type errors.
	  //default size/strenght; you can tweak this as see fit

		int index = (int)sim->magnets.size(); //index before adding
		sim->addMagnet(Magnet(x, y, 0.0, 150.0, 80.0, 20.0)); 
		return index; 
	}

	void RemoveMagnet(int index)
	{
		sim->removeMagnet(index);
	}
};
