...
public:
	...
	std::vector<Vec2> traceLine(const Vec2& start, bool forward, int ignoreIndex) const
	{
		std::vector<Vec2> pts;
		...
		{
			double Bx, By;
			sim->computeField(p.x, p.y, Bx, By,ignoreIndex);
	...
	std::vector<std::vector<Vec2>> generateFieldLinesPerMagnet() const
	{
		...
		for (int mi = 0; mi < (int)sim->magnets.size(); mi++)
		{
			const auto& m = sim->magnets[mi]; 
			...
			{
				double t = (2.0 * 3.141596 * k) / linesPerMagnet;
				...
				auto lineForward = traceLine(seed, true, mi);
				auto lineBackward = traceLine(seed, false, mi);
